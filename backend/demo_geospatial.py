...
It produces geospatial_demo.png and geospatial_demo_route.geojson.
"""
...
OUTPUT_PNG = Path("geospatial_demo.png")
OUTPUT_GEOJSON = Path("geospatial_demo_route.geojson")
...
def main() -> None:
    ...
    for row in range(grid.rows):
        for col in range(grid.cols):
            lat, lon = grid.node_to_coord(grid.__class__.__dict__.get('dummy', None)) if False else grid.node_to_coord(GridNode(row, col))
            xs.append(lon)
            ys.append(lat)
